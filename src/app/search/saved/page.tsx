...
export default async function SavedSearchesPage() {
  ...
  if (!user) redirect("/login?next=/search/saved");
  ...
  const rows = await Promise.all(
    baseRows.map(async (s) => {
      ...
      // Always try to generate the URL from the path first (most reliable)
      if (s.reference_image_path) {
        try {
          const { data } = (await supabase.storage.from(bucket).getPublicUrl(s.reference_image_path));
          imageUrl = data.publicUrl;
        } catch (error) {
          console.error('Failed to generate image URL for path:', s.reference_image_path, error);
        }
      }
      
      // Fallback to stored URL if path-based generation failed
      if (!imageUrl && s.reference_image_url) {
        imageUrl = s.reference_image_url;
      }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Link href="/search" className="rounded border px-3 py-2">Back to Search</Link>
      </div>
