...
export default async function SavedSearchDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!user) redirect(`/login?next=/research/saved/${id}`);

  const { data, error } = await supabase
    .from("saved_searches")
    .select("id,title,query,ai_query,reference_image_url,results_json,created_at")
    .eq("id", id)
    .single();
    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href="/research" className="rounded border px-3 py-2">Research</Link>
          <Link href="/research/saved" className="rounded border px-3 py-2">All Saved</Link>
        </div>
      </div>
